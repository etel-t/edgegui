...
class CustomLoginView(LoginView):
    ...
    success_url = reverse_lazy('contact')  # Redirect to contact page after login
...
def contact(request):
    if request.method == 'POST':
        ...
        if form.is_valid():        
                ...
                if reg_data["registration_response"] == "Successfully Registered" or  reg_data["registration_response"] == "This device is already Registered":
                    ...
                    if date_object > today_date:
                        ...
                        if (get_interface_details()):
                            ...
                            return render(request, 'error_config.html')
                    else:
            ...
            return render(request, 'conf_error.html')
        elif form2.is_valid():                                 
            ...
            if protocol_opt == "DHCP":
                intfc_info = vpp_int_details()
                for intfc in intfc_info:
                    if intfc["int_name"] == "GigabitEthernet0/9/0":
                        if intfc['int_ipv4_address'] != 'none':
                            os.system(f"sudo vppctl set int state GigabitEthernet0/9/0 up")
                            os.system(f"sudo vppctl set int ip address del GigabitEthernet0/9/0 {intfc['int_ipv4_address']}")
                            os.system(f"sudo vppctl set dhcp client intfc GigabitEthernet0/9/0")             
                            ...
                            os.system(f"sudo vppctl set int state GigabitEthernet0/9/0 up")
                            os.system(f"vppctl set dhcp client intfc GigabitEthernet0/9/0")
                            os.system(f"sudo vppctl lcp create GigabitEthernet0/9/0 host-if Reach_int3")  
                            ...
                            with open("/etc/vpp/bootstrap.vpp", "r")as f:
                                data_boot = f.read()                                
                                f.close() 
                            if "set dhcp client intfc GigabitEthernet0/9/0" not in data_backup: 
                                data_backup = data_backup + f"\nset dhcp client intfc GigabitEthernet0/9/0\nlcp create GigabitEthernet0/9/0 host-if Reach_int3"   
                                data_boot = data_boot + f"\nset dhcp client intfc GigabitEthernet0/9/0\nlcp create GigabitEthernet0/9/0 host-if Reach_int3"                         
                        response = {"message":"IPAddress changed successfully"}
                        countdown_timer(20)
                        gateway1_opt = get_default_route_info()
                        if gateway1_opt != None:
                            with open("/etc/frr/frr.conf", "a")as f:
                                f.write(f"\n!\nip route 0.0.0.0/0 {gateway1_opt}\n!\n")
                                f.close()                                                         
                            os.system("sudo service frr restart")                             
                        return render(request, 'conf_success.html')   
            ip_addr_opt = form3.cleaned_data['ip_address_opt']
            netmask_opt = form3.cleaned_data['netmask_opt']
            gateway_opt = form3.cleaned_data.get('gateway_opt') 
            pri_dns_opt = form3.cleaned_data.get('primary_dns_opt') 
            sec_dns_opt = form3.cleaned_data.get('secondary_dns_opt') 
            if pri_dns_opt == '':
                pri_dns_opt = '8.8.8.8'
            if sec_dns_opt == '':
                sec_dns_opt = '8.8.4.4'
                    ...
                    if intfc['int_ipv4_address'] != 'none':
                        ...
                        if retvalue.retval == 0:  
                            ...
                            with open("/etc/vpp/bootstrap.vpp", "r")as f:
                                data_boot = f.read()
                                f.close()
                            data_backup = data_backup.replace(f"set int ip address GigabitEthernet0/9/0 {intfc['int_ipv4_address']}", f"set int ip address {intfc_details[0].interface_name} {ip_address_opt}")
                            data_backup = data_backup.replace(f"set dhcp client intfc GigabitEthernet0/9/0", f"set int ip address GigabitEthernet0/9/0 {ip_address_opt}")
                            data_boot = data_boot.replace(f"set int ip address GigabitEthernet0/9/0 {intfc['int_ipv4_address']}", f"set int ip address GigabitEthernet0/9/0 {ip_address_opt}")
                            data_boot = data_boot.replace(f"set dhcp client intfc GigabitEthernet0/9/0", f"set int ip address GigabitEthernet0/9/0 {ip_address}")
                            
                            if gateway_opt != "" and 'gateway' in intfc:
                                data_backup = data_backup.replace(f"ip route add 0.0.0.0/0 via {intfc['gateway']}", f"ip route add 0.0.0.0/0 via {gateway_opt}")
                                data_boot = data_boot.replace(f"ip route add 0.0.0.0/0 via {intfc['gateway']}", f"ip route add 0.0.0.0/0 via {gateway_opt}")
                            with open("/etc/vpp/backup.vpp", "w")as f:
                                f.write(data_backup)
                                f.close()    
                            with open("/etc/vpp/bootstrap.vpp", "w")as f:
                                ...
                                f.close()
                            if gateway_opt != '':
                                os.system(f"sudo vppctl ip route add 0.0.0.0/0 via {gateway_opt}")
                                with open("/etc/frr/frr.conf", 'a') as f:
                                    f.write(f"\n!\niproute 0.0.0.0/0 {gateway_opt}\n!\n") 
                                    f.close() 
                            return render(request, 'conf_success.html') 
                    else:
                        os.system("sudo vppctl set int state GigabitEthernet0/9/0 up")
                        os.system(f"sudo vppctl set int ip address GigabitEthernet0/9/0 {ip_address_opt}") 
                        os.system(f"sudo vppctl lcp create GigabitEthernet0/9/0 host-if Reach_int3")                         
                        if gateway_opt != '':
                            os.system(f"sudo vppctl ip route add 0.0.0.0/0 via {gateway_opt}")
                            with open("/etc/frr/frr.conf", 'a') as f:
                            ...
                            os.system("sudo systemctl restart openvpn@client")  
                        with open("/etc/vpp/backup.vpp", "a")as f:
                            f.write(f"\nset int ip address GigabitEthernet0/9/0 {ip_address_opt}\nlcp create GigabitEthernet0/9/0 host-if Reach-int3\nlcplcp-sync on")
                            f.close()    
                        with open("/etc/vpp/bootstrap.vpp", "a")as f:
                            f.write(f"\nset int ip address GigabitEthernet0/9/0 {ip_address_opt}\nlcp create GigabitEthernet0/9/0 host-if Reach-int3\nlcp lcp-sync on")
                            f.close()                             
        ...
        if os.path.exists("/etc/reach/register_info.json"):
            if  check_tunnel_connection() == False:
                ...
                initial_reg_data = {"status": "Your device is linked with ReachManage",
                                    "name" : " ",
                                    "Registered_mail": " ",
                                    "password": " ",
                                    "branch_location": " "                                    
        ...
        opt_intfc_info = coll_vpp_interface_details.find_one({"interface_name":"Reach_int3"}) 
        initial_opt_data = {}
        if opt_intfc_info is not None:
            initial_opt_data = {"ip_address_opt": opt_intfc_info.get("IPv4address_noprefix", ""),
                                ...
                                "primary_dns_opt": primary_dns,
                                "secondary_dns_opt":sec_dns,
                                "protocol_opt":opt_intfc_info.get("protocol", "")
                                }        
        form1 = NetworkSettingsForm(initial = initial_wan_data)
    ...
    return render(request, 'dashboard.html', {'form': form, 'form1':form1, 'form2': form2, 'logfile_content':logfile_content, 'form3': form3, 'form4':form4, 'form5': form5, 'status': reg_status, 'routing_table':routing_table, 'form6':form6, 'form7':form7})
